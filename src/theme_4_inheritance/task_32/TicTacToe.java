...
public class TicTacToe {
    ...
    private char chooseFirstPlayer() {
        ...
        if (firstPlayer != 'X' && firstPlayer != '0') {
            throw new IncorrectInputException(" Enter X or 0!");
        }
        return firstPlayer;
    }
    ...
    private boolean isValidMove(int row, int col) {
        if (row < 1 || row > SIZE || col < 1 || col > SIZE) {
            if (row < 1 || row > SIZE) {
                throw new IncorrectInputException(" Line number should be (1-" + SIZE + "): ");
            } else {
                throw new IncorrectInputException(" Column number should be (1-" + SIZE + "): ");
            }
        } else if (boardArray[row - 1][col - 1] == 'X' || boardArray[row - 1][col - 1] == '0') {
            throw new IncorrectInputException(" The selected cell is already occupied!");
        }
        return true;
    }
