...
public class ItunesPreviewPlayer {
    ...
    void play(String searchRequest, int limit, String mediaParam) throws IOException {
        String url = buildUrl(searchRequest, limit, mediaParam);

        String page = downloadPage.downloadWebPage(url);

        String name = getTagString(page, "trackName");
    ...
    public static String getTagString(String page, String tagName) {
        int start = page.indexOf(tagName) + tagName.length() + 3;
        int end = page.indexOf("\"", start);
        return page.substring(start, end);
    }

    public static String getTagNumbers(String page, String tagName) {
        int start = page.indexOf(tagName) + tagName.length() + 2;
        int end = page.indexOf(",", start);
        return page.substring(start, end);
    }

    public static String buildUrl(String searchRequest, int limit, String mediaName) {
